#################
###  Diff between Rebecca Turner's taxonomy file and the authoritative taxononmy table
################
...
# only returns two rows of differences, but unintelligable result

################################################

# Use another method to Diff the two files: 

diff1 <- setdiff(rt_aug_file, tax_table)
str(diff1)
...
anti_join(tax_table, rt_aug_file, by = "taxon_id")

diff3 <- anti_join(tax_table, rt_aug_file)

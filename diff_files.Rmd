...
rt_aug_file <- rt_aug_file %>% arrange(taxon_id) %>% mutate_all(~gsub("(*UCP)\\s\\+|\\W+$", "", . , perl=TRUE))
...
```{r}
tt_sub2 <- tax_table %>% select(rank, family, genus_species) %>% filter(rank == "species")
rt2 <- rt_aug_file %>% select(rank, family , genus_species) %>% filter(rank == "species")
